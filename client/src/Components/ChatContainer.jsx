  ...
  return  selectedUser ? (
        <div className="h-full overflow-scroll relative backdrop-blur-lg"> 
          ...
          <div className="flex items-centre gap-3 py-3 mx-4 border-b border-stone-500">
            ...
            <p className="flex-1 text-lg text-white flex items-centre gap-2">Vishal Singh
                ...
                {messagesDummyData.map((msg,index)=>(
                  <div key={index} className={`flex items-end gap-2 justify-end ${msg.senderId !== '680f50e4f10f3cd28382ecf9' && 'flex-row-reverse'}`}>

                    {
                      ...
                      border border-gray-700 rounded-1g overflow-hidden mb-8  "/>
                    ):(
                      <p className={`p-2 max-w-[200px] md:text-sm font-light 
                        rounded-1g mb-8 break-all bg-violet-500/30 text-white ${msg.senderId === '680f50e4f10f3cd28382ecf9'? 'rounded-br-none'
                          : 'rounded-b1-none'}`}>{msg.text}</p>
                    )}
